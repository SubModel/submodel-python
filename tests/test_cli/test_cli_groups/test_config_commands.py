...
class TestCommands(unittest.TestCase):
    ...
    def test_config_wizard(self):
        ...
        with patch("click.echo") as mock_echo, patch(
            "submodel.cli.groups.config.commands.set_credentials"
        ) as mock_set_credentials, patch(
            "submodel.cli.groups.config.commands.check_credentials"
            ...
            result = self.runner.invoke(
                submodel_cli, ["config", "--profile", "test", "KEY"]
            )
            ...
            result = self.runner.invoke(submodel_cli, ["config", "--profile", "test"])
            assert result.exit_code == 0
            mock_set_credentials.assert_called_with("KEY", "test", overwrite=True)
            mock_prompt.assert_called_with(
                "    > SubModel API Key", hide_input=False, confirmation_prompt=False
    ...
    def test_check_flag(self):
        ...
        with patch(
            ...
            mock_check_creds.return_value = (True, None)
            result = self.runner.invoke(
                submodel_cli, ["config", "--check", "--profile", "test"]
            )
            assert result.exit_code == 0
    ...
    def test_output_messages(self):
        ...
        with patch("click.echo") as mock_echo, patch(
            ...
            result = self.runner.invoke(
                submodel_cli, ["config", "KEY", "--profile", "test"]
            )
            mock_set_credentials.assert_called_with("KEY", "test", overwrite=True)
            mock_echo.assert_any_call(
                "Credentials set for profile: test in ~/.submodel/config.toml"
            )
            assert result.exit_code == 0
