...
from submodel.cli.groups.ssh.commands import add_key, list_keys
...
class TestSSHCommands(unittest.TestCase):
    ...
    def test_list_keys(self):
        ...
        runner = CliRunner()
        with patch(
            "submodel.cli.groups.ssh.commands.get_user_pub_keys",
            return_value=[
    ...
    def test_add_key_without_params(self):
        ...
        runner = CliRunner()
        with patch(
            "submodel.cli.groups.ssh.commands.generate_ssh_key_pair"
        ) as mock_gen_key, patch(
            "submodel.cli.groups.ssh.commands.click.prompt", return_value="TestKey"
        ) as mock_prompt, patch(
            "submodel.cli.groups.ssh.commands.click.confirm", return_value=True
