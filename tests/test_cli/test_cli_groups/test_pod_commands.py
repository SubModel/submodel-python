...
from submodel.cli.entry import submodel_cli


class TestPodCommands(unittest.TestCase):
    """Test CLI pod commands"""

    @patch("submodel.cli.groups.pod.commands.get_pods")
    @patch("submodel.cli.groups.pod.commands.click.echo")
    def test_list_pods(self, mock_echo, mock_get_pods):
        ...
        runner = CliRunner()
        result = runner.invoke(submodel_cli, ["pod", "list"])
    ...
    @patch("submodel.cli.groups.pod.commands.click.prompt")
    @patch("submodel.cli.groups.pod.commands.click.confirm")
    @patch("submodel.cli.groups.pod.commands.click.echo")
    @patch("submodel.cli.groups.pod.commands.create_pod")
    def test_create_new_pod(
        ...
        mock_prompt.return_value = "SubModel-CLI-Pod"
        mock_create_pod.return_value = {"id": "sample_id"}

        runner = CliRunner()
        result = runner.invoke(submodel_cli, ["pod", "create"])

        # Assertions
        assert result.exit_code == 0, result.exception
        mock_prompt.assert_called_once_with("Enter pod name", default="SubModel-CLI-Pod")
    ...
    @patch("submodel.cli.groups.pod.commands.click.echo")
    @patch("submodel.cli.groups.pod.commands.ssh_cmd.SSHConnection")
    def test_connect_to_pod(self, mock_ssh_connection, mock_echo):
        ...
        runner = CliRunner()
        result = runner.invoke(submodel_cli, ["pod", "connect", pod_id])
    ...
    @patch("submodel.cli.groups.pod.commands.os.unlink")
    @patch("submodel.cli.groups.pod.commands.tempfile.NamedTemporaryFile")
    @patch("submodel.cli.groups.pod.commands.uuid.uuid4")
    @patch("submodel.cli.groups.pod.commands.click.echo")
    @patch("submodel.cli.groups.pod.commands.ssh_cmd.SSHConnection")
    def test_sync_pods_success(self, mock_ssh_connection, mock_echo, mock_uuid, mock_temp_file, mock_unlink):
        ...
        with patch("submodel.cli.groups.ssh.functions.get_user_pub_keys") as mock_get_keys:
            mock_get_keys.return_value = [{"name": "test-key", "type": "ssh-rsa", "fingerprint": "SHA256:test"}]
            
            runner = CliRunner()
            result = runner.invoke(submodel_cli, ["pod", "sync", "source_pod", "dest_pod", "/workspace", "/workspace"])
    ...
    def test_sync_pods_no_ssh_keys(self, mock_ssh_connection, mock_echo, mock_uuid):
        ...
        with patch("submodel.cli.groups.ssh.functions.get_user_pub_keys") as mock_get_keys:
            ...
            runner = CliRunner()
            result = runner.invoke(submodel_cli, ["pod", "sync", "source_pod", "dest_pod"])

        assert result.exit_code == 0, result.exception
        
        # Verify error message was shown
        mock_echo.assert_any_call("❌ No SSH keys found in your SubModel account!")
        mock_echo.assert_any_call("🔑 To create an SSH key, run:")
        mock_echo.assert_any_call("   submodel ssh add-key")
    ...
    def test_sync_pods_source_not_found(self, mock_ssh_connection, mock_echo, mock_uuid):
        ...
        with patch("submodel.cli.groups.ssh.functions.get_user_pub_keys") as mock_get_keys:
            ...
            runner = CliRunner()
            result = runner.invoke(submodel_cli, ["pod", "sync", "source_pod", "dest_pod", "/nonexistent", "/workspace"])
