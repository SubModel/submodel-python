""" Test functions in submodel.cli.groups.project.functions module. """
...
from submodel.cli.groups.project.functions import (
    STARTER_TEMPLATES,
    create_new_project,
...
class TestCreateNewProject(unittest.TestCase):
    ...
    @patch("submodel.cli.groups.project.functions.copy_template_files")
    ...
    def test_create_new_project_init_current_dir(
        ...
        mock_file_open.assert_called_with(
            "/tmp/testdir/submodel.toml", "w", encoding="UTF-8"
        )
        assert mock_getcwd.called
    ...
    def test_replace_placeholders_in_handler(
        ...
        with patch("submodel.cli.groups.project.functions.copy_template_files"):
            create_new_project(
    ...
    @patch("os.path.exists", return_value=False)
    @patch("builtins.open", new_callable=mock_open)
    def test_create_submodel_toml(self, mock_open_file, mock_exists):
        """Test that submodel.toml configuration file is created."""
        with patch("submodel.cli.groups.project.functions.copy_template_files"):
            create_new_project("test_project", "volume_id", "11.8.0", "3.8")
        toml_file_location = os.path.join(os.getcwd(), "test_project", "submodel.toml")
    ...
    @patch("os.path.exists", return_value=True)
    @patch("builtins.open", new_callable=mock_open, read_data="<<SUBMODEL>> placeholder")
    def test_update_requirements_file(self, mock_open_file, mock_exists):
        ...
        with patch("submodel.cli.groups.project.functions.__version__", "dev"), patch(
            "submodel.cli.groups.project.functions.copy_template_files"
        ):
            create_new_project("test_project", "volume_id", "11.8.0", "3.8")
        assert mock_open_file.called
        assert mock_exists.called
    ...
    def test_update_requirements_file_non_dev(self, mock_open_file, mock_exists):
        ...
        with patch("submodel.cli.groups.project.functions.__version__", "1.0.0"), patch(
            "submodel.cli.groups.project.functions.copy_template_files"
        ):
            create_new_project("test_project", "volume_id", "11.8.0", "3.8")
        assert mock_open_file.called
...
class TestStartProject(unittest.TestCase):
    ...
    @patch("submodel.cli.groups.project.functions.load_project_config")
    @patch("submodel.cli.groups.project.functions.get_project_pod")
    @patch("submodel.cli.groups.project.functions.attempt_pod_launch")
    @patch("submodel.cli.groups.project.functions.get_pod")
    @patch("submodel.cli.groups.project.functions.SSHConnection")
    @patch("os.getcwd", return_value="/current/path")
    def test_start_nonexistent_successfully(
        ...
        with patch(
            "submodel.cli.groups.project.functions.sync_directory"
    ...
    def test_failed_pod_launch(self, mock_attempt_pod, mock_get_pod):
        ...
        with patch("builtins.print") as mock_print, patch(
            "submodel.cli.groups.project.functions.load_project_config"
        ):
...
class TestStartProjectAPI(unittest.TestCase):
    ...
    @patch("submodel.cli.groups.project.functions.SSHConnection")
    @patch("os.getcwd", return_value="/current/path")
    @patch("submodel.cli.groups.project.functions.sync_directory")
...
class TestCreateProjectEndpoint(unittest.TestCase):
    ...
    @patch("submodel.cli.groups.project.functions.SSHConnection")
    @patch("submodel.cli.groups.project.functions.load_project_config")
    @patch("submodel.cli.groups.project.functions.create_template")
    @patch("submodel.cli.groups.project.functions.create_endpoint")
    @patch("submodel.cli.groups.project.functions.update_endpoint_template")
    @patch("submodel.cli.groups.project.functions.get_project_pod")
    @patch("submodel.cli.groups.project.functions.get_project_endpoint")
    def test_create_project_endpoint(
        ...
        with patch(
            "submodel.cli.groups.project.functions._launch_dev_pod"
        ...
        mock_load_project_config.return_value = {
            "project": {
                "name": "test_project",
                "volume_mount_path": "/submodel-volume/123456",
                "uuid": "123456",
        ...
        with patch("submodel.cli.groups.project.functions.datetime") as mock_datetime:
        ...
        mock_create_template.assert_called_with(
            ...
            docker_start_cmd='bash -c ". /submodel-volume/123456/prod/venv/bin/activate && python -u /submodel-volume/123456/prod/test_project/handler.py"',  # pylint: disable=line-too-long
            env={"TEST_VAR": "value"},
            is_serverless=True,
        )
