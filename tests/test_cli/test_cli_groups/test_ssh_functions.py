...
from submodel.cli.groups.ssh.functions import (
    add_ssh_key,
...
class TestSSHFunctions(unittest.TestCase):
    ...
    @patch("submodel.cli.groups.ssh.functions.get_user")
    ...
    def test_add_ssh_key_already_exists(self, mock_get_user):
        ...
        key = "ssh-rsa AAAAB3Nza...base64data...Q== user@host"
        with patch(
            "submodel.cli.groups.ssh.functions.update_user_settings"
    ...
    @patch("submodel.cli.groups.ssh.functions.os.path.join")
    @patch("submodel.cli.groups.ssh.functions.paramiko.RSAKey.generate")
    def test_generate_ssh_key_pair(self, mock_generate, mock_path_join):
        ...
        with patch("os.mkdir") as mock_mkdir, patch(
            ...
            "submodel.cli.groups.ssh.functions.os.chmod"
        ) as mock_chmod, patch(
            "submodel.cli.groups.ssh.functions.add_ssh_key"
        ) as mock_add_key:
    ...
    @patch("submodel.cli.groups.ssh.functions.get_user")
    @patch("submodel.cli.groups.ssh.functions.update_user_settings")
