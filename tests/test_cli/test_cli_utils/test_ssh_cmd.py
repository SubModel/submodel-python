"""
SubModel | CLI | Utils | SSH Command
"""
...
class TestSSHConnection(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patch_get_pod_ssh_ip_port = patch(
            "submodel.cli.utils.ssh_cmd.get_pod_ssh_ip_port",
            return_value=("127.0.0.1", 22),
        ).start()

        self.patch_find_ssh_key_file = patch(
            "submodel.cli.utils.ssh_cmd.find_ssh_key_file", return_value="key_file"
        ).start()

        self.mock_ssh_client = MagicMock()
        patch_paramiko = patch(
            "submodel.cli.utils.ssh_cmd.paramiko.SSHClient",
    ...
    @patch("submodel.cli.utils.ssh_cmd.SSHConnection.close")
