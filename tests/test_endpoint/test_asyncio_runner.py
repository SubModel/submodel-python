...
from submodel.endpoint.asyncio.asyncio_runner import Endpoint, Job
...
class TestJob(IsolatedAsyncioTestCase):
    ...
    async def test_output(self):
        ...
        """
        with patch(
            "submodel.endpoint.asyncio.asyncio_runner.asyncio.sleep"
        ) as mock_sleep, patch(
            "aiohttp.ClientSession", new_callable=AsyncMock
        ) as mock_session_class:
            mock_session = mock_session_class.return_value
...
class TestEndpointInitialization(unittest.TestCase):
    ...
    def test_endpoint_initialization(self):
        ...
        with patch("aiohttp.ClientSession"):
            ...
            self.assertEqual(
                endpoint.endpoint_url, "https://api.submodel.ai/v1/sl/endpoint_id/run"
            )
