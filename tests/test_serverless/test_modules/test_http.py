...
from submodel.serverless.modules import sm_http
...
class TestHTTP(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_send_result(self):
        ...
        """
        with patch("submodel.serverless.modules.sm_http.log") as mock_log, patch(
            "submodel.serverless.modules.sm_http.RetryClient"
        ) as mock_retry:
            ...
            send_return_local = await sm_http.send_result(
    ...
    async def test_send_result_type_error(self):
        ...
        """
        with patch("submodel.serverless.modules.sm_http.log") as mock_log, patch(
            "submodel.serverless.modules.sm_http.json.dumps"
        ) as mock_dumps, patch(
            "submodel.serverless.modules.sm_http.RetryClient"
        ) as mock_retry:
    ...
    async def test_stream_result(self):
        ...
        with patch("submodel.serverless.modules.sm_http.log") as mock_log, patch(
            ...
            send_return_local = await sm_http.stream_result(
                AsyncMock(), self.job_data, self.job
            )
