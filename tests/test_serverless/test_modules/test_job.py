...
from submodel.http_client import TooManyRequests
from submodel.serverless.modules import sm_job
...
class TestJob(IsolatedAsyncioTestCase):
    ...
    async def test_get_job_200(self):
        ...
        with patch("aiohttp.ClientSession") as mock_session, patch(
            "submodel.serverless.modules.sm_job.JOB_GET_URL", "http://mock.url"
        ):
            mock_session.get.return_value.__aenter__.return_value = response
            job = await sm_job.get_job(mock_session)
    ...
    async def test_get_job_429(self):
        ...
        with patch("aiohttp.ClientSession") as mock_session, patch(
            ...
            with self.assertRaises(ClientResponseError):
                await sm_job.get_job(mock_session)
...
class TestRunJob(IsolatedAsyncioTestCase):
    ...
    async def test_simple_job(self):
        ...
        job_result = await sm_job.run_job(mock_handler, self.sample_job)
        assert job_result == {"output": "test"}

        mock_handler.return_value = ["test1", "test2"]
        job_result_list = await sm_job.run_job(mock_handler, self.sample_job)
        assert job_result_list == {"output": ["test1", "test2"]}

        mock_handler.return_value = 123
        job_result_int = await sm_job.run_job(mock_handler, self.sample_job)
...
class TestRunJobGenerator(IsolatedAsyncioTestCase):
    ...
    async def test_run_job_generator_success(self):
        ...
        with patch(
            "submodel.serverless.modules.sm_job.log", new_callable=Mock
        ) as mock_log:
            result = [i async for i in sm_job.run_job_generator(handler, job)]
