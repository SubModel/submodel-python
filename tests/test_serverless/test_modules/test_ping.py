...
from submodel.serverless.modules.sm_ping import Heartbeat
...
class TestHeartbeat:
    ...
    def mock_env(self):
        ...
        env_vars = {
            "SUBMODEL_WEBHOOK_PING": "https://test.com/ping/$SUBMODEL_POD_ID",
            "SUBMODEL_AI_API_KEY": "test_api_key",
            "SUBMODEL_POD_ID": "test_pod_id",
            "SUBMODEL_PING_INTERVAL": "5000"
        }
    ...
    def mock_worker_id(self):
        ...
        with patch("submodel.serverless.modules.sm_ping.WORKER_ID", "test_worker_123"):
            yield "test_worker_123"

    @pytest.fixture
    def mock_session(self):
        """Mock the SyncClientSession"""
        with patch("submodel.serverless.modules.sm_ping.SyncClientSession") as mock:
    ...
    def mock_jobs(self):
        ...
        with patch("submodel.serverless.modules.sm_ping.JobsProgress") as mock:
    ...
    def mock_logger(self):
        """Mock the logger"""
        with patch("submodel.serverless.modules.sm_ping.log") as mock:
            yield mock
    ...
    def test_start_ping_missing_api_key(self, mock_logger, mock_worker_id):
        ...
        with patch.dict(os.environ, {"SUBMODEL_POD_ID": "test", "SUBMODEL_WEBHOOK_PING": "test"}, clear=True):
            with patch("multiprocessing.Process") as mock_process:
                ...
                mock_logger.debug.assert_called_with(
                    "Not deployed on SubModel serverless, pings will not be sent."
                )

    def test_start_ping_missing_pod_id(self, mock_logger, mock_worker_id):
        """Test start_ping when POD_ID is missing"""
        with patch.dict(os.environ, {"SUBMODEL_AI_API_KEY": "test"}, clear=True):
            with patch("multiprocessing.Process") as mock_process:
                heartbeat = Heartbeat()
                heartbeat.start_ping()
                ...
                mock_logger.info.assert_called_with(
                    "Not running on SubModel, pings will not be sent."
                )

    def test_start_ping_missing_webhook_url(self, mock_logger, mock_worker_id):
        """Test start_ping when webhook URL is not set"""
        with patch.dict(os.environ, {"SUBMODEL_AI_API_KEY": "test", "SUBMODEL_POD_ID": "test"}, clear=True):
            with patch("multiprocessing.Process") as mock_process:
                heartbeat = Heartbeat()
                heartbeat.start_ping()
    ...
    @patch("submodel.serverless.modules.sm_ping.Process")
    @patch("submodel.serverless.modules.sm_ping.SyncClientSession")
    @patch("submodel.serverless.modules.sm_ping.WORKER_ID", "test_worker_123")
    ...
    def test_send_ping_success(self, mock_env, mock_worker_id, mock_session, mock_jobs, mock_logger):
        ...
        # Mock version
        with patch("submodel.serverless.modules.sm_ping.submodel_version", "1.0.0"):
            heartbeat._send_ping()
        
        # Verify request was made correctly
        mock_session.get.assert_called_once_with(
            "https://test.com/ping/test_worker_123",
            params={"job_id": "job1,job2,job3", "submodel_version": "1.0.0"},
    ...
    def test_send_ping_no_jobs(self, mock_env, mock_worker_id, mock_session, mock_logger):
        ...
        # Mock no jobs
        with patch("submodel.serverless.modules.sm_ping.JobsProgress.get_job_list", return_value=None):
            mock_response = MagicMock()
            mock_response.url = "https://test.com/ping/test_worker_123"
            mock_response.status_code = 200
            mock_session.get.return_value = mock_response
            ...
            mock_session.get.assert_called_once_with(
                "https://test.com/ping/test_worker_123",
                params={"job_id": None, "submodel_version": "1.0.0"},
                timeout=10
            )
