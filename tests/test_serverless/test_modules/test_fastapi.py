""" Tests for submodel.serverless.modules.sm_fastapi.py """
...
import submodel
from submodel.serverless.modules import sm_fastapi
...
class TestFastAPI(unittest.TestCase):
    ...
    def test_start_serverless_with_realtime(self):
        ...
        """
        module_location = "submodel.serverless.modules.sm_fastapi"
        with patch(
            ...
            sm_fastapi.SUBMODEL_REALTIME_PORT = "1111"
            sm_fastapi.SUBMODEL_ENDPOINT_ID = "test_endpoint_id"

            os.environ["SUBMODEL_REALTIME_PORT"] = "1111"
            os.environ["SUBMODEL_ENDPOINT_ID"] = "test_endpoint_id"

            submodel.serverless.start({"handler": self.handler})
            ...
            self.assertTrue(sm_fastapi.SUBMODEL_ENDPOINT_ID == "test_endpoint_id")
            self.assertTrue(mock_router.return_value.add_api_route.called)

            self.assertTrue(mock_uvicorn.run.called)

            os.environ.pop("SUBMODEL_REALTIME_PORT")
    ...
    def test_webhook_sender_success(self):
        ...
        module_location = "submodel.serverless.modules.sm_fastapi.requests.Session.post"
        ...
        with patch(module_location, new_callable=MagicMock) as mock_post:
            ...
            # Call the function
            success = sm_fastapi._send_webhook("test_webhook", {"test": "output"})
    ...
    def test_run(self):
        ...
        with patch(
            ...
            job_object = sm_fastapi.Job(
                id="test_job_id", input={"test_input": "test_input"}
            )

            default_input_object = sm_fastapi.DefaultRequest(
                input={"test_input": "test_input"}
            )

            # Test with handler
            worker_api = sm_fastapi.WorkerAPI({"handler": self.handler})
            ...
            generator_worker_api = sm_fastapi.WorkerAPI({"handler": generator_handler})
    ...
    def test_runsync(self):
        ...
        with patch(f"{module_location}.FastAPI", Mock()), patch(
            ...
            input_object_with_webhook = sm_fastapi.DefaultRequest(
            ...
            error_worker_api = sm_fastapi.WorkerAPI({"handler": self.error_handler})
