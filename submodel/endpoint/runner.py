"""
SubModel | Python | Endpoint Runner
"""
...
from submodel.endpoint.helpers import (
    API_KEY_NOT_SET_MSG,
    FINAL_STATES,
    UNAUTHORIZED_MSG,
    is_completed,
...
class SubModelClient:
    ...
    def __init__(self):
        """
        Initialize a SubModelClient instance.

        Raises:
            RuntimeError: If the API key has not been initialized.
        """
        from submodel import (  # pylint: disable=import-outside-toplevel, cyclic-import
            api_key,
            endpoint_url_base,
        )
        ...
        self.sm_session = requests.Session()
        retries = Retry(total=5, backoff_factor=1, status_forcelist=[408, 429])
        self.sm_session.mount("http://", HTTPAdapter(max_retries=retries))
...
# ---------------------------------------------------------------------------- #
class Job:
    """Represents a job to be run on the SubModel service."""

    def __init__(self, endpoint_id: str, job_id: str, client: SubModelClient):
        """
        ...
        Args:
            ...
            client: An instance of the SubModelClient to make requests with.
        """
    ...
    def _fetch_job(self, source: str = "status") -> Dict[str, Any]:
        ...
        job_state = self.sm_client.get(endpoint=status_url)
...
# ---------------------------------------------------------------------------- #
class Endpoint:
    """Manages an endpoint to run jobs on the SubModel service."""
    ...
    def run(self, request_input: Dict[str, Any]) -> Job:
        ...
        job_request = self.sm_client.post(f"{self.endpoint_id}/run", request_input)
        return Job(self.endpoint_id, job_request["id"], self.sm_client)
    ...
    def run_sync(
        ...
        job_request = self.sm_client.post(
        ...
        return Job(self.endpoint_id, job_request["id"], self.sm_client).output(
            timeout=timeout
        )
    ...
    def health(self, timeout: int = 3) -> Dict[str, Any]:
        ...
        Args:
            timeout: The number of seconds to wait for the server to respond before giving up.
        """
        return self.sm_client.get(f"{self.endpoint_id}/health", timeout=timeout)
