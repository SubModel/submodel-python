...
from .sm_job import run_job, run_job_generator
from .sm_ping import Heartbeat
from .worker_state import Job, JobsProgress

SUBMODEL_ENDPOINT_ID = os.environ.get("SUBMODEL_ENDPOINT_ID", None)

TITLE = "SubModel | Development Worker API"

DESCRIPTION = """
The Development Worker API facilitates testing and debugging of your SubModel workers.
It offers a sandbox environment for executing code and simulating interactions with your worker, ensuring your applications can seamlessly transition to production on SubModel serverless platform.
Use this API for comprehensive testing of request submissions and result retrieval, mimicking the behavior of SubModel's operational environment.
---
*Note: This API serves as a local testing tool and will not be utilized once your worker is operational on the SubModel platform.*
"""

# Add CLI tool suggestion if SUBMODEL_PROJECT_ID is not set.
if os.environ.get("SUBMODEL_PROJECT_ID", None) is None:
    DESCRIPTION += """

    ℹ️ | Consider developing with our CLI tool to streamline your worker development process.

    >_  wget -qO- cli.submodel.ai | sudo bash
    >_  submodelctl project create
    """
...
class WorkerAPI:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if SUBMODEL_ENDPOINT_ID:
            api_router.add_api_route(
                f"/{SUBMODEL_ENDPOINT_ID}/realtime", self._realtime, methods=["POST"]
            )
        ...
        self.sm_app.include_router(api_router)
