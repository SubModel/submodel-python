...
if platform.system() == "Linux" or platform.system() == "Darwin": # Darwin is macOS
    ...
    print("Warning: fcntl not available on this OS. Using fallback for worker_state.")

import tempfile
from typing import Any, Dict, Optional, Set

from .sm_logger import SubModelLogger
...
WORKER_ID = os.environ.get("SUBMODEL_POD_ID", str(uuid.uuid4()))


# ----------------------------------- Flags ---------------------------------- #
IS_LOCAL_TEST = os.environ.get("SUBMODEL_WEBHOOK_GET_JOB", None) is None
...
class JobsProgress(Set[Job]):
    ...
    _STATE_FILE = os.path.join(_STATE_DIR, ".submodel_jobs.pkl")
        ...
        try:
            if (
                ...
                with open(self._STATE_FILE, "rb") as f:
                    if FCNTL_OPEN: fcntl.flock(f, fcntl.LOCK_SH)
                    try:
                        ...
                        if FCNTL_OPEN: fcntl.flock(f, fcntl.LOCK_UN)
    ...
    def _save_state(self):
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                ...
                if FCNTL_OPEN: fcntl.flock(temp_f, fcntl.LOCK_EX)
                try:
                    pickle.dump(set(self), temp_f)
                finally:
                    if FCNTL_OPEN: fcntl.flock(temp_f, fcntl.LOCK_UN)
