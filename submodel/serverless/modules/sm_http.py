...
from submodel.http_client import ClientSession
...
JOB_DONE_URL_TEMPLATE = str(
    os.environ.get("SUBMODEL_WEBHOOK_POST_OUTPUT", "JOB_DONE_URL")
)
JOB_DONE_URL = JOB_DONE_URL_TEMPLATE.replace("$SUBMODEL_POD_ID", WORKER_ID)

JOB_STREAM_URL_TEMPLATE = str(
    os.environ.get("SUBMODEL_WEBHOOK_POST_STREAM", "JOB_STREAM_URL")
)
JOB_STREAM_URL = JOB_STREAM_URL_TEMPLATE.replace("$SUBMODEL_POD_ID", WORKER_ID)
