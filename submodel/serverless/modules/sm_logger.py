...
class SubModelLogger:
    """Singleton class for logging."""

    __instance = None
    level = _validate_log_level(
        os.environ.get(
            "SUBMODEL_LOG_LEVEL", os.environ.get("SUBMODEL_DEBUG_LEVEL", "DEBUG")
        )
    )

    def __new__(cls):
        if SubModelLogger.__instance is None:
            SubModelLogger.__instance = object.__new__(cls)
        return SubModelLogger.__instance
    ...
    def log(self, message, message_level="INFO", job_id=None):
        """
        Log message to stdout if SUBMODEL_DEBUG is true.
        """
        ...
        if os.environ.get("SUBMODEL_ENDPOINT_ID"):
