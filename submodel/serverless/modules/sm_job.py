...
from submodel.http_client import ClientSession, TooManyRequests
from submodel.serverless.modules.sm_logger import SubModelLogger

from ...version import __version__ as submodel_version
from ..utils import sm_debugger
from .sm_handler import is_generator
from .sm_http import send_result, stream_result
from .sm_tips import check_return_size
from .worker_state import WORKER_ID, REF_COUNT_ZERO, JobsProgress

JOB_GET_URL = str(os.environ.get("SUBMODEL_WEBHOOK_GET_JOB")).replace("$ID", WORKER_ID)
...
def _job_get_url(batch_size: int = 1):
    ...
    log.debug(f"sm_job | get_job: {job_take_url}")
    return job_take_url
...
async def get_job(
    ...
    async with session.get(_job_get_url(num_jobs)) as response:
        log.debug(f"sm_job | Response: {type(response).__name__} {response.status}")

        if response.status == 204:
            log.debug("sm_job | Received 204 status, no jobs.")
            return

        if response.status == 400:
            log.debug("sm_job | Received 400 status, expected when FlashBoot is enabled.")
            return
        ...
        if response.content_type != "application/json":
            log.debug(f"sm_job | Unexpected content type: {response.content_type}")
            return

        # Check if there is a non-empty content to parse
        if response.content_length == 0:
            log.debug("sm_job | No content to parse.")
            return

        try:
            jobs = await response.json()
            log.debug("sm_job | Received Job(s)")
        except aiohttp.ContentTypeError:
            log.debug(f"sm_job | Response content is not valid JSON. {response.content}")
            return
        except ValueError as json_error:
            log.debug(f"sm_job | Failed to parse JSON response: {json_error}")
            return
...
async def handle_job(session: ClientSession, config: Dict[str, Any], job) -> dict:
    ...
    # If sm_debugger is set, debugger output will be returned.
    if config.get("sm_args", {}).get("sm_debugger", False) and isinstance(job_result, dict):
        job_result["output"]["sm_debugger"] = sm_debugger.get_debugger_output()
        log.debug("sm_debugger | Flag set, returning debugger output.", job["id"])

        # Calculate ready delay for the debugger output.
        ready_delay = (config["reference_counter_start"] - REF_COUNT_ZERO) * 1000
        job_result["output"]["sm_debugger"]["ready_delay_ms"] = ready_delay
    else:
        log.debug("sm_debugger | Flag not set, skipping debugger output.", job["id"])
        sm_debugger.clear_debugger_output()
...
async def run_job(handler: Callable, job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        error_info = {
            ...
            "hostname": os.environ.get("SUBMODEL_POD_HOSTNAME", "unknown"),
            "worker_id": os.environ.get("SUBMODEL_POD_ID", "unknown"),
            "submodel_version": submodel_version,
        }
