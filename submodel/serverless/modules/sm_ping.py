...
The heartbeat is responsible for sending periodic pings to the SubModel server.
"""
...
from submodel.http_client import SyncClientSession
from submodel.serverless.modules.sm_logger import SubModelLogger
from submodel.serverless.modules.worker_state import WORKER_ID, JobsProgress
...
class Heartbeat:
    """Sends heartbeats to the SubModel server."""
    ...
    def __init__(self, pool_connections=10, retries=3) -> None:
        ...
        """
        self.PING_URL = os.environ.get("SUBMODEL_WEBHOOK_PING", "PING_NOT_SET")
        self.PING_URL = self.PING_URL.replace("$SUBMODEL_POD_ID", WORKER_ID)
        self.PING_INTERVAL = int(os.environ.get("SUBMODEL_PING_INTERVAL", 10000)) // 1000

        # Create a new HTTP session
        self._session = SyncClientSession()
        self._session.headers.update(
            {"Authorization": os.environ.get("SUBMODEL_AI_API_KEY", "")}
        )
    ...
    def start_ping(self, test=False):
        """
        Sends heartbeat pings to the SubModel server in a separate process.
        """
        if not os.environ.get("SUBMODEL_AI_API_KEY"):
            log.debug("Not deployed on SubModel serverless, pings will not be sent.")
            return

        if not os.environ.get("SUBMODEL_POD_ID"):
            log.info("Not running on SubModel, pings will not be sent.")
            return
    ...
    def ping_loop(self, test=False):
        """
        Sends heartbeat pings to the SubModel server until interrupted.
        """
        while True:
            self._send_ping()
    ...
    def _send_ping(self):
        """
        Sends a heartbeat to the SubModel server.
        """
        jobs = JobsProgress()  # Get the singleton instance
        job_ids = jobs.get_job_list()
        ping_params = {"job_id": job_ids, "submodel_version": submodel_version}
