"""
submodel | serverless | sm_local.py
Provides the local testing functionality for submodel serverless worker.
"""
...
async def run_local(config: Dict[str, Any]) -> None:
    ...
    if config["sm_args"].get("test_input", None):
        ...
        local_job = config["sm_args"]["test_input"]
    else:
    ...
    if config["sm_args"].get("test_output", None):
        ...
        if job_result != config["sm_args"]["test_output"]:
