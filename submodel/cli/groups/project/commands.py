...
from submodel import get_user

from .functions import create_new_project, create_project_endpoint, start_project
from .helpers import validate_project_name


@click.group("project", help="Create, deploy, and manage SubModel projects.")
def project_cli():
    """Create and deploy projects on SubModel."""
...
def new_project_wizard(project_name, model_type, model_name, init_current_dir):
    ...
    if len(network_volumes) == 0:
        ...
        click.echo(
            "Please create a network volume (https://submodel.ai/#/pvc/list) and try again."
    ...
    submodel_volume_id = cli_select(questions)["volume-id"]
    ...
    click.echo(f"   - SubModel Network Storage ID: {submodel_volume_id}")
    ...
    click.echo("")
    click.echo(
        "From your project root run `submodel project start` to start a development pod."
    )
...
def start_project_pod():
    """
    Start the project development pod from submodel.toml
    """
    click.echo(
        "Starting the project will create a development pod on SubModel, if one does not already exist."
    )  # pylint: disable=line-too-long
    click.echo(
        "    - You will be charged based on the GPU type specified in submodel.toml."
    )
...
def deploy_project():
    """Deploy the project to SubModel."""
    ...
    click.echo(f"    - https://api.submodel.ai/v1/sl/{endpoint_id}/runsync")
    click.echo(f"    - https://api.submodel.ai/v1/sl/{endpoint_id}/run")
    click.echo(f"    - https://api.submodel.ai/v1/sl/{endpoint_id}/health")
