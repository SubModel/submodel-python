"""
SubModel | CLI | Project | Functions
"""
...
from submodel.cli import BASE_DOCKER_IMAGE, ENV_VARS, GPU_TYPES
from submodel.cli.utils.ssh_cmd import SSHConnection

from ...utils.sm_sync import sync_directory
from .helpers import (
    attempt_pod_launch,
    copy_template_files,
...
def _launch_dev_pod():
    ...
    config = load_project_config()  # Load submodel.toml

    print("Deploying development pod on SubModel...")

    # Prepare the environment variables
    environment_variables = {"SUBMODEL_PROJECT_ID": config["project"]["uuid"]}
...
def create_new_project(
    ...
    if not init_current_dir:
        ...
        if "dev" in __version__:
            requirements_content = requirements_content.replace(
                "<<SUBMODEL>>", "git+https://github.com/submodel/submodel-python.git"
            )
        else:
            requirements_content = requirements_content.replace(
                "<<SUBMODEL>>", f"submodel=={__version__}"
            )
    ...
    toml_config.add(comment("SubModel Project Configuration"))
    toml_config.add(nl())
    ...
    project_table.add("storage_id", submodel_volume_id)
    project_table.add("volume_mount_path", "/submodel-volume")
    ...
    with open(
        os.path.join(project_folder, "submodel.toml"), "w", encoding="UTF-8"
    ) as config_file:
...
def start_project():  # pylint: disable=too-many-locals, too-many-branches
    """
    Start the project development environment from submodel.toml
    ...
    with SSHConnection(project_pod_id) as ssh_conn:
        ...
        launch_api_server = [
            ...
            python {handler_path} --sm_serve_api --sm_api_host="0.0.0.0" --sm_api_port=8080 --sm_api_concurrency=1 &
            last_pid=$!

            echo -e "- Started API server with PID: $last_pid" && echo ""
            echo "Connect to the API server at:"
            echo ">  https://$SUBMODEL_POD_ID-8080.tun.submodel.ai" && echo ""
            ...
            while true; do
                ...
                if [[ $changed_file == *".submodelignore"* ]]; then
                    update_exclude_pattern
                fi
...
def create_project_endpoint():
    ...
    activate_cmd = (
        f'. /submodel-volume/{config["project"]["uuid"]}/prod/venv/bin/activate'
    )
    python_cmd = f'python -u /submodel-volume/{config["project"]["uuid"]}/prod/{config["project"]["name"]}/{config["runtime"]["handler_path"]}'  # pylint: disable=line-too-long
