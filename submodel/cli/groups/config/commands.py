...
@click.command("config", help="Configures the SubModel CLI with the user's API key.")
...
def config_wizard(api_key, profile, check):
    """Starts the configuration wizard to set up the SubModel CLI.
    ...
    if api_key is None:
        click.echo("Please enter your Submodel API Key.")
        click.echo("You can find it at https://submodel.ai/#/account/others")
    ...
    click.echo(f"Credentials set for profile: {profile} in ~/.submodel/config.toml")
