"""
submodel | cli | config.py

A collection of functions to set and validate configurations.
Configurations are TOML files located under ~/.submodel/
"""
...
CREDENTIAL_FILE = os.path.expanduser("~/.submodel/config.toml")


def set_credentials(api_key: str, profile: str = "default", overwrite=False) -> None:
    """
    Sets the user's credentials in ~/.submodel/config.toml
...
def check_credentials(profile: str = "default"):
    ...
    if not os.path.exists(CREDENTIAL_FILE):
        return False, "~/.submodel/config.toml does not exist."
    ...
    try:
        ...
        if profile not in config:
            return False, f"~/.submodel/config.toml is missing {profile} profile."

        if "api_key" not in config[profile]:
            return (
                False,
                f"~/.submodel/config.toml is missing api_key for {profile} profile.",
            )

    except (TypeError, ValueError):
        return False, "~/.submodel/config.toml is not a valid TOML file."
...
def get_credentials(profile="default"):
    """
    Returns the credentials for the specified profile from ~/.submodel/config.toml
    """
    if not os.path.exists(CREDENTIAL_FILE):
        return None
