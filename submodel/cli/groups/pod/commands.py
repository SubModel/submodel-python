...
from submodel import create_pod, get_pods
...
def create_new_pod(
    ...
    if not name:
        name = click.prompt("Enter pod name", default="SubModel-CLI-Pod")

    quick_launch = click.confirm("Would you like to launch default pod?", abort=True)
    if quick_launch:
        image = "submodel/base:0.0.0"
...
def sync_pods(source_pod_id, dest_pod_id, source_workspace, dest_workspace):
    ...
    1. SSH Key Setup:
       • You must have an SSH key configured in your SubModel account
       • If you don't have one, create it with: submodel ssh add-key
       • List your keys with: submodel ssh list-keys
    
    2. Pod Configuration:
       • Both pods must have SSH access enabled
       • For running pods using official SubModel templates, you may need to add
    ...
    Basic sync (uses /workspace as default):
        submodel pod sync pod1 pod2
    
    Custom paths:
        submodel pod sync pod1 pod2 /workspace/data /workspace/backup
    
    Different directories:
        submodel pod sync pod1 pod2 /home/user/files /workspace/imported
    """
    ...
    try:
        ...
        if not user_keys:
            click.echo("❌ No SSH keys found in your SubModel account!")
            click.echo("")
            click.echo("🔑 To create an SSH key, run:")
            click.echo("   submodel ssh add-key")
            click.echo("")
            click.echo("📖 For more help, see:")
            click.echo("   submodel ssh add-key --help")
            return
        else:
    ...
    try:
        ...
        with ssh_cmd.SSHConnection(dest_pod_id) as dest_ssh:
            ...
            click.echo(f"   submodel ssh {dest_pod_id}")
        ...
        click.echo("• Check that your SSH key is added to your SubModel account: submodel ssh list-keys")
